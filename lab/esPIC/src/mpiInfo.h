...
class mpiInfo
{
 public:
  ...
  void PEsum(VD &field){
    ...
    if (nei_s >= 0) iLOOP phiSend_s[i] = field[pid(i, 1)];
    if (nei_w >= 0) jLOOP phiSend_w[j] = field[pid(1, j)];
    if (nei_e >= 0) jLOOP phiSend_e[j] = field[pid(nRealx, j)];
    if (nei_n >= 0) iLOOP phiSend_n[i] = field[pid(i , nRealy)];

    if (nei_ne >= 0) send_ne = field[pid(nRealx, nRealy)];
    if (nei_nw >= 0) send_nw = field[pid(1, nRealy)];
    if (nei_se >= 0) send_se = field[pid(nRealx, 1)];
    if (nei_sw >= 0) send_sw = field[pid(1, 1)];
    ...
    if (nei_ne >= 0) err = MPI_Isend(&send_ne, 1, MPI_DOUBLE, nei_ne, 0, MPI_COMM_WORLD, &request);
    if (nei_nw >= 0) err = MPI_Isend(&send_nw, 1, MPI_DOUBLE, nei_nw, 0, MPI_COMM_WORLD, &request);
    if (nei_se >= 0) err = MPI_Isend(&send_se, 1, MPI_DOUBLE, nei_se, 0, MPI_COMM_WORLD, &request);
    if (nei_sw >= 0) err = MPI_Isend(&send_sw, 1, MPI_DOUBLE, nei_sw, 0, MPI_COMM_WORLD, &request);
    ...
    if (nei_ne >= 0) {err = MPI_Irecv(&rec_ne, 1, MPI_DOUBLE, nei_ne, 0, MPI_COMM_WORLD, &request); MPI_Wait(&request,&status);} 
    if (nei_nw >= 0) {err = MPI_Irecv(&rec_nw, 1, MPI_DOUBLE, nei_nw, 0, MPI_COMM_WORLD, &request); MPI_Wait(&request,&status);}
    if (nei_se >= 0) {err = MPI_Irecv(&rec_se, 1, MPI_DOUBLE, nei_se, 0, MPI_COMM_WORLD, &request); MPI_Wait(&request,&status);}
    if (nei_sw >= 0) {err = MPI_Irecv(&rec_sw, 1, MPI_DOUBLE, nei_sw, 0, MPI_COMM_WORLD, &request); MPI_Wait(&request,&status);}

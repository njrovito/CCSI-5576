...
void SolveLinearSystem(int max_iter , VD RHS, VD &Solution , mpiInfo &myMPI )
  {
	...
	myMPI.ExchangeBoundaryInfo(Solution, b);  // b is a member of this class 
	...
	int root = 0;
	err = MPI_Reduce( &converged, &global_converged, one , MPI_INT , MPI_MIN , root, MPI_COMM_WORLD );
	err = MPI_Bcast ( &global_converged, one, MPI_INT, root, MPI_COMM_WORLD);
